...
/** Media generation config for voice/video delivery. */
...
async function deliver(ctx: Context, response: OutgoingMessage, media: TelegramMediaConfig = {}): Promise<void> {
  ...
  if (response.mediaPath && fs.existsSync(response.mediaPath)) {
    try {
      ...
      if (VIDEO_EXTS.has(ext)) {
        // Try to send as a video note (кружочек) first, fall back to regular video
        try {
  ...
  if (mode === "voice") {
    const sent = await sendVoiceResponse(ctx as never, response.text, media.voiceConfig ?? {}, media.falApiKey);
    if (!sent) await replyHtml(ctx, response.text);
    return;
  }

  if (mode === "video") {
    const avatarPath = typeof media.avatarPath === "function" ? media.avatarPath() : (media.avatarPath ?? "");
    const sent = await sendVideoNote(ctx as never, response.text, media.voiceConfig ?? {}, media.falApiKey ?? "", avatarPath);
    if (!sent) await replyHtml(ctx, response.text);
    return;
  }
...
export function registerHandlers(
  ...
  mediaConfig?: TelegramMediaConfig,
  allowAll?: boolean,
): void {
  // --- Owner-only filter ---
  ...
  async function handleWithTyping(
    ...
    try {
      ...
      await deliver(ctx, modeOverride ? { ...response, mode: modeOverride } : response, mediaConfig ?? {});
    } catch (err) {
      stopTyping();
      if (draftTimer) clearTimeout(draftTimer);
      if (statusMsgId) {
  ...
  bot.command("start", async (ctx) => {
    ...
    const hello = firstName ? `Здравствуй, ${escapeHtml(firstName)}! 😊` : "Здравствуй! 😊";
    await ctx.reply(
      `${hello}\n\n` +
      `Я — Ава, твой персональный AI-компаньон. Могу поговорить, помочь с задачами и прислать селфи или видео-кружочек.\n\n` +
      `Команды:\n` +
      `🎥 <code>/video текст</code> — видео-кружочек с лип-синком\n` +
      `🎙 <code>/voice текст</code> — голосовое сообщение\n` +
      `📸 <code>/selfie описание</code> — сгенерировать селфи\n` +
      `🖼 <code>/setphoto</code> — задать фото-референс (ответь фот на неё)\n` +
      `💬 Можно просто писать — я отвечаю и на голосовые тоже.\n\n` +
      `Что я умею:\n\n` +
      `💬 <b>Общение</b>\n` +
      `• Веду живой диалог, помню контекст и детали нашего разговора\n` +
      `• Отвечаю на голосовые — могу говорить в ответ голосом\n` +
      `• Подстраиваюсь под тон: от лёгкой беседы до серьёзной поддержки\n\n` +
      `🎥 <b>Видео-кружочки</b>\n` +
      `• Снимаю «кружочек» с твоим текстом и липсингом — как будто говорю на камеру (команда /video)\n\n` +
      `🎙 <b>Голосовые</b>\n` +
      `• Зачитываю любой текст голосом (команда /voice)\n` +
      `• Годится для поздравлений, заметок и озвучки\n\n` +
      `📸 <b>Селфи и изображения</b>\n` +
      `• Генерирую селфи по твоему описанию: образ, обстановка, настроение (команда /selfie)\n` +
      `• Запоминаю твой внешний вид через фото-референс, чтобы снимки были похожи на тебя (команда /setphoto)\n\n` +
      `🧠 <b>Помощь с задачами</b>\n` +
      `• Пишу тексты: письма, посты, объявления, поздравления, резюме\n` +
      `• Помогаю с идеями и мозговым штурмом: нейминг, сценарии, концепции\n` +
      `• Редактирую написанное — сделаю короче, длиннее или в другом стиле\n` +
      `• Объясняю сложное простым языком — от школьных тем до рабочих вопросов\n` +
      `• Перевожу с/на разные языки\n` +
      `• Помогаю с планированием: списки задач, расписание, чек-листы\n` +
      `• Даю советы и второе мнение по бытовым и личным вопросам`,
      { parse_mode: "HTML" },
    );
  });
  // /status
  ...
  // Voice messages — transcribe and reply (in voice mode so it talks back)
  bot.on(["message:voice", "message:audio"], async (ctx) => {
    const voice = (ctx as Context).message?.voice ?? (ctx as Context).message?.audio;
    if (!voice) return;

    const falKey = mediaConfig?.falApiKey;
    if (!falKey) {
      await ctx.reply("Для распознавания голосовых нужен ключ fal.ai.");
      return;
    }
    ...
    try {
      ...
      // Ask to keep typing indicator while transcribing
      const stopTyping = startTyping(ctx);
      const text = await transcribeSpeech(buffer, falKey);
      stopTyping();
      ...
      // Reply in voice mode so the bot talks back
      await handleWithTyping(ctx, text, (mediaConfig?.voiceConfig && falKey) ? "voice" : "text");
    } catch (err) {
